...
NET_BUF_SIMPLE_DEFINE_STATIC(sensor_data_1, 2);

static esp_ble_mesh_sensor_state_t sensor_states[2] = {
    /* Mesh Model Spec:
     * Multiple instances of the Sensor states may be present within the same
     * model, provided that each instance has a unique value of the Sensor
     * Property ID to allow the instances to be differentiated. Such sensors are
     * known as multisensors. In this example, two instances of the Sensor
     * states within the same model are provided.
     */
    [0] =
        {
            /* Mesh Model Spec:
             * Sensor Property ID is a 2-octet value referencing a device
             * property that describes the meaning and format of data reported
             * by a sensor. 0x0000 is prohibited.
             */
            .sensor_property_id = SENSOR_PROPERTY_ID_0,
            /* Mesh Model Spec:
             * Sensor Descriptor state represents the attributes describing the
             * sensor data. This state does not change throughout the lifetime
             * of an element.
             */
            .descriptor =
                {
...
struct example_sensor_descriptor {
    ...
    uint32_t pos_tolerance : 12, neg_tolerance : 12, sample_func : 8;
    uint8_t measure_period;
    uint8_t update_interval;
} __attribute__((packed));

static void example_ble_mesh_send_sensor_descriptor_status(
    ...
    status = (uint8_t *)calloc(
        1, ARRAY_SIZE(sensor_states) * ESP_BLE_MESH_SENSOR_DESCRIPTOR_LEN);
    if (!status) {
    ...
    if (param->value.get.sensor_descriptor.op_en == false) {
        /* Mesh Model Spec:
         * Upon receiving a Sensor Descriptor Get message with the Property ID
         * field omitted, the Sensor Server shall respond with a Sensor
         * Descriptor Status message containing the Sensor Descriptor states for
         * all sensors within the Sensor Server.
         */
     ...
     * contain the requested Property ID value and the other fields of the
     * Sensor Descriptor state shall be omitted.
     */
...
static void example_ble_mesh_send_sensor_cadence_status(
    esp_ble_mesh_sensor_server_cb_param_t *param) {
    esp_err_t err;

    /* Sensor Cadence state is not supported currently. */
    err = esp_ble_mesh_server_model_send_msg(
        param->model, &param->ctx, ESP_BLE_MESH_MODEL_OP_SENSOR_CADENCE_STATUS,
        ESP_BLE_MESH_SENSOR_PROPERTY_ID_LEN,
...
static void example_ble_mesh_send_sensor_settings_status(
    esp_ble_mesh_sensor_server_cb_param_t *param) {
    esp_err_t err;

    /* Sensor Setting state is not supported currently. */
    err = esp_ble_mesh_server_model_send_msg(
        param->model, &param->ctx, ESP_BLE_MESH_MODEL_OP_SENSOR_SETTINGS_STATUS,
        ESP_BLE_MESH_SENSOR_PROPERTY_ID_LEN,
...
static void example_ble_mesh_send_sensor_setting_status(
    esp_ble_mesh_sensor_server_cb_param_t *param) {
    ...
    setting.sensor_setting_prop_id =
        param->value.get.sensor_setting.setting_property_id;

    err = esp_ble_mesh_server_model_send_msg(
        param->model, &param->ctx, ESP_BLE_MESH_MODEL_OP_SENSOR_SETTING_STATUS,
...
static uint16_t example_ble_mesh_get_sensor_data(
    esp_ble_mesh_sensor_state_t *state, uint8_t *data) {
    ...
    if (state->sensor_data.length == ESP_BLE_MESH_SENSOR_DATA_ZERO_LEN) {
        /* For zero-length sensor data, the length is 0x7F, and the format is
         * Format B. */
        mpid = ESP_BLE_MESH_SENSOR_DATA_FORMAT_B_MPID(
            state->sensor_data.length, state->sensor_property_id);
        mpid_len = ESP_BLE_MESH_SENSOR_DATA_FORMAT_B_MPID_LEN;
        data_len = 0;
    } else {
        if (state->sensor_data.format == ESP_BLE_MESH_SENSOR_DATA_FORMAT_A) {
            mpid = ESP_BLE_MESH_SENSOR_DATA_FORMAT_A_MPID(
                state->sensor_data.length, state->sensor_property_id);
        ...
        /* Use "state->sensor_data.length + 1" because the length of sensor data
         * is zero-based. */
...
static void example_ble_mesh_send_sensor_status(
    esp_ble_mesh_sensor_server_cb_param_t *param) {
    uint8_t *status = NULL;
    uint16_t buf_size = 0;
    uint16_t length = 0;
     ...
     * |--------Field--------|-Size
     * (octets)-|------------------------Notes-------------------------|
     * |----Property ID 1----|-------2-------|--ID of the 1st device property of
     * the sensor---------|
     * |-----Raw Value 1-----|----variable---|--Raw Value field defined by the
     * 1st device property--|
     * |----Property ID 2----|-------2-------|--ID of the 2nd device property of
     * the sensor---------|
     * |-----Raw Value 2-----|----variable---|--wld defined by the
     * 2nd device property--| | ...... |
     * |----Property ID n----|-------2-------|--ID of the nth device property of
     * the sensor---------|
     * |-----Raw Value n-----|----variable---|--Raw Value field defined by the
     * nth device property--|
     */
    for (i = 0; i < ARRAY_SIZE(sensor_states); i++) {
        ...
        if (state->sensor_data.length == ESP_BLE_MESH_SENSOR_DATA_ZERO_LEN) {
            ...
            /* Use "state->sensor_data.length + 1" because the length of sensor
             * data is zero-based. */
    ...
    if (param->value.get.sensor_data.op_en == false) {
        /* Mesh Model Spec:
         * If the message is sent as a response to the Sensor Get message, and
         * if the Property ID field of the incoming message is omitted, the
         * Marshalled Sensor Data field shall contain data for all device
         * properties within a sensor.
         */
        for (i = 0; i < ARRAY_SIZE(sensor_states); i++) {
            length += example_ble_mesh_get_sensor_data(&sensor_states[i],
                                                       status + length);
        }
        goto send;
    }
    ...
    /* Mesh Model Spec:
     * Otherwise, the Marshalled Sensor Data field shall contain data for the
     * requested device property only.
     */
    for (i = 0; i < ARRAY_SIZE(sensor_states); i++) {
        if (param->value.get.sensor_data.property_id ==
            sensor_states[i].sensor_property_id) {
            length =
                example_ble_mesh_get_sensor_data(&sensor_states[i], status);
            goto send;
        }
    }

    /* Mesh Model Spec:
     * Or the Length shall represent the value of zero and the Raw Value field
     * shall contain only the Property ID if the requested device property is
     * not recognized by the Sensor Server.
     */
    mpid = ESP_BLE_MESH_SENSOR_DATA_FORMAT_B_MPID(
...
send:
    ...
    err = esp_ble_mesh_server_model_send_msg(
        param->model, &param->ctx, ESP_BLE_MESH_MODEL_OP_SENSOR_STATUS, length,
        status);
    if (err != ESP_OK) {
...
static void example_ble_mesh_send_sensor_column_status(
    esp_ble_mesh_sensor_server_cb_param_t *param) {
    uint8_t *status = NULL;
    uint16_t length = 0;
    esp_err_t err;
    ...
    memcpy(status, &param->value.get.sensor_column.property_id,
           ESP_BLE_MESH_SENSOR_PROPERTY_ID_LEN);
    memcpy(status + ESP_BLE_MESH_SENSOR_PROPERTY_ID_LEN,
           param->value.get.sensor_column.raw_value_x->data,
           param->value.get.sensor_column.raw_value_x->len);

    err = esp_ble_mesh_server_model_send_msg(
        param->model, &param->ctx, ESP_BLE_MESH_MODEL_OP_SENSOR_COLUMN_STATUS,
        length, status);
    if (err != ESP_OK) {
...
static void example_ble_mesh_send_sensor_series_status(
    esp_ble_mesh_sensor_server_cb_param_t *param) {
    esp_err_t err;

    err = esp_ble_mesh_server_model_send_msg(
        param->model, &param->ctx, ESP_BLE_MESH_MODEL_OP_SENSOR_SERIES_STATUS,
        ESP_BLE_MESH_SENSOR_PROPERTY_ID_LEN,
...
static void example_ble_mesh_sensor_server_cb(
    esp_ble_mesh_sensor_server_cb_event_t event,
    esp_ble_mesh_sensor_server_cb_param_t *param) {
    ESP_LOGI(
        TAG, "Sensor server, event %d, src 0x%04x, dst 0x%04x, model_id 0x%04x",
...
static esp_err_t ble_mesh_init(void) {
    ...
    esp_ble_mesh_register_sensor_server_callback(
        example_ble_mesh_sensor_server_cb);
...
void server_send_to_client() {
    ...
    err = esp_ble_mesh_server_model_send_msg(
        sensor_server.model, &_ctx, ESP_BLE_MESH_MODEL_OP_SENSOR_STATUS, length,
        status);
    if (err != ESP_OK) {
        ESP_LOGE(TAG, "Failed to send Sensor Status");
    }
...
void DHT_readings(uint16_t *temp, uint16_t *hum) {
    setDHTgpio(GPIO_NUM_21);
    int ret = readDHT();
    errorHandler(ret);
    *temp = (uint16_t)(roundf(getTemperature() * 100));
    *hum = (uint16_t)(roundf(getHumidity() * 100));
}

void update_sensor_data(uint16_t temp, uint16_t hum) {
    // Chuyển đổi giá trị uint16_t sang dạng byte
    uint8_t data0_bytes[2];
    uint8_t data1_bytes[2];

    // Đảm bảo dữ liệu là Little Endian
    data0_bytes[0] = temp & 0xFF;
    data0_bytes[1] = (temp >> 8) & 0xFF;

    data1_bytes[0] = hum & 0xFF;
    data1_bytes[1] = (hum >> 8) & 0xFF;

    // Làm sạch bộ đệm trước khi thêm dữ liệu mới
    net_buf_simple_reset(&sensor_data_0);
    net_buf_simple_reset(&sensor_data_1);

    // Thêm dữ liệu vào bộ đệm
    net_buf_simple_add_mem(&sensor_data_0, data0_bytes, sizeof(data0_bytes));
    net_buf_simple_add_mem(&sensor_data_1, data1_bytes, sizeof(data1_bytes));
    server_send_to_client();
}

void send_sensor_data_over_ble_mesh(uint16_t temp, uint16_t hum) {
    // Thêm logic để gửi dữ liệu qua BLE Mesh ở đây
    update_sensor_data(temp, hum);
    ESP_LOGI("BLE Mesh", "Sending Temperature: %d, Humidity: %d", temp, hum);
    ESP_LOGI("BLE Mesh", "Sending Temperature Hex: 0x%x, Humidity: 0x%x", temp,
             hum);
}
...
void sensor_task(void *pvParameter) {
    ...
    while (1) {
        if (provisioned == true) {
            DHT_readings(&temp, &hum);  // Read data from Sensor DHT22
            send_sensor_data_over_ble_mesh(temp,
                                           hum);     // Send data
            vTaskDelay(60000 / portTICK_PERIOD_MS);  // Wait 1h to Send
        } else {
